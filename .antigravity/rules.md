...
2.  **Wichtig:** Hinterlege alle Schema-Anpassungen (Tabellen, Spalten, Constraints) vorläufig **immer direkt** im Haupt-Skript `./flutter_brew_assistent/db_scripts/full/001_init_schema.sql`. Vermeide separate Inkrement-Skripte, um eine konsolidierte Basis zu behalten.
3.  Führe das Init-Script aus: `./flutter_brew_assistent/db_scripts/full/001_init_schema.sql`
4.  Führe direkt danach das Seed-Script aus: `./flutter_brew_assistent/db_scripts/full/aibrewgenius_seed.sql`

## 4. Abschluss (Restart)
Sobald Code und Datenbank-Status validiert sind:
1.  Stoppe die aktuell laufende Instanz der App (SIGTERM/Stop).
2.  Starte die App frisch neu via `flutter run -d web-server --web-port=8080` (dies startet den Server ohne Browser-Fenster).